...
import type { RoofConfig } from "@/types/interfaces"
import { roofMaterials, insulationMaterials } from "@/data/materials"

interface RoofTabProps {
  data: RoofConfig
  onChange: (data: Partial<RoofConfig>) => void
}

export default function RoofTab({ data, onChange }: RoofTabProps) {
  const selectedMaterial = roofMaterials.find((m) => m.id === data.material)
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <Label htmlFor="roof-thickness">Roof Thickness (meters)</Label>
        <Input
          id="roof-thickness"
          type="number"
          min="0.1"
      ...
      <div>
        <Label htmlFor="roof-insulation">Roof Insulation</Label>
        ...
        {selectedInsulation && (
          <p className="text-xs text-gray-500 mt-1">Conductivity: {selectedInsulation.thermalConductivity} W/m·K</p>
        )}
      </div>

      <div>
        <Label htmlFor="roof-insulation-thickness">Insulation Thickness (meters)</Label>
        <Input
